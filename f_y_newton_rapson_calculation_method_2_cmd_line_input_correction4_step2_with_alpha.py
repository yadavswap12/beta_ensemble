# -*- coding: utf-8 -*-
"""
Created on Wed Jun 24 17:09:39 2020
...
epsi = 1e-4
alpha = 0.2    #alpha is the rate of descend towards minima also known as learning rate(use small alpha to avoid overshooting the solution)
...
f_x_input = numpy.loadtxt("input_output_files/rho_"+str(rho)+"/theta_"+str(theta)+"/gamma_"+str(gamma)+"/density/f_y_calculation_newton_rapson_method_2_gamma="+str(gamma)+"_max_err_1e-10_theta="+str(theta)+"_18000points_correction4_iter"+str(iteration-1)+"_with_alpha.txt",float)
f_out=file("input_output_files/rho_"+str(rho)+"/theta_"+str(theta)+"/gamma_"+str(gamma)+"/density/f_y_calculation_newton_rapson_method_2_gamma="+str(gamma)+"_max_err_1e-10_theta="+str(theta)+"_18000points_correction4_iter"+str(iteration)+"_with_alpha.txt","w")
...
delta_X = alpha*numpy.linalg.solve(jacobian,F)   
